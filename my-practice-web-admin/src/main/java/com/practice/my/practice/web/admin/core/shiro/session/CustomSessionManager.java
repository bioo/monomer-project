...
import com.practice.my.practice.web.admin.core.shiro.CustomShiroSessionDAO;
import com.practice.my.practice.web.admin.dto.UserOnlineDto;
...
public class CustomSessionManager {
    ...
    public List<UserOnlineDto> getAllUser() {
        ...
        for (Session session : sessions) {
            UserOnlineDto dto = getSessionBo(session);
            if (null != dto) {
                list.add(dto);
            }
        }
    ...
    public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long... userIds) {
        ...
        for (Session session : sessions) {
            ...
            if (null != obj && obj instanceof SimplePrincipalCollection) {
                ...
                if (null != obj && obj instanceof TbUser) {
    ...
    private UserOnlineDto getSessionBo(Session session) {
        ...
        if (obj instanceof SimplePrincipalCollection) {
            ...
            if (null != obj && obj instanceof TbUser) {
                //存储session + user 综合信息
                UserOnlineDto userDto = new UserOnlineDto((TbUser) obj);
                //最后一次和系统交互的时间
    ...
    public void forbidUserById(Long id, Long status) {
        //获取所有在线用户
        for (UserOnlineDto Dto : getAllUser()) {
            Long userId = Dto.getId();
            //匹配用户ID
            if (userId.equals(id)) {
                //获取用户Session
                Session session = shiroSessionRepository.getSession(Dto.getSessionId());
                //标记用户Session
                SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
